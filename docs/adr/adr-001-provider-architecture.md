...
TennoDex uses a provider-based architecture for player data synchronisation. Providers are swappable implementations of platform-specific data sources (Overwolf GEP, JSON import, manual mark, etc.) that all emit a standardised `IncomingPlayerSnapshot`.
...
2. **Platform independence** — TennoDex ships in two editions (OW-Electron and standalone Electron). Providers abstract the difference so the engine and UI never know which edition is running.
